...
module Compiler =
  ...
  let VMValueKind =function
      | VValue _ -> 0
      | VFunc _ -> 1

  let OpCodeKind = function
      | OpPop -> 0
      | OpPush _ -> 1
      | OpAllocLvars _ -> 2
      | OpFreeVars -> 3
  ...
  let rec compileSexpObject sexp =
    ...
    | List (Symbol x :: xs) when x = "def-var" ->
      if List.length xs = 2 then
        let e1 = List.item 0 xs |> function
          | Symbol v -> v
    ...
    | List (Symbol x :: xs) when x = "def-fun" ->
      if List.length xs = 3 then
        let funcName = List.item 0 xs |> function
          | Symbol v -> v
          | _ -> failInvalid (Some "def-fun<1>")
        let e2 = List.item 1 xs |> function
          | List l -> l
        ...
        let lvars = ref []
        let funcBody' =
          let isOpSetVar = function
            | OpSetVar _ -> true
            | _ -> false
          let isOpVarDef = function
            | OpVarDef _ -> true
            | _ -> false
          List.map (fun op ->
              match op with
              ...
              | OpSetVar varName ->
                ...
                let found = List.tryFindIndex (fun e -> e = varName) argNames
                let argNamesLen = List.length argNames
                begin match found with
                  | None ->
                    if isOpVarDef op || isOpSetVar op then
                      let lvarIdx1 =
                        if isOpVarDef op then begin
              ...
              | _ -> op) funcBody
        let allocaSize = List.length argNames + List.length !lvars
        let funcBody = ref []
        ...
        if allocaSize > 0 then
          listAddRef funcBody OpFreeVars
    ...
    | List (Symbol x :: xs) when x = "begin" ->
      if List.length xs >= 1 then
        (List.map compileSexpObject >> List.concat) xs
      else
        failInvalid (Some "begin")
    | List (Symbol x :: xs) when x = "while" ->
      if List.length xs = 2 then
        ...
        let jmpOffset =
          -(
            OpCodesLen exprIns + 2 (* this jump *) +
            OpCodesLen condIns + 2 (* branch *)
          )
        let exprIns = exprIns @ [OpJumpRel jmpOffset]

        let condWithBranch = List.append condIns [OpBranch (OpCodesLen exprIns)]
        List.append condWithBranch exprIns
      else
        failInvalid (Some "while")
    | List (Symbol x :: xs) when x = "set" ->
      if List.length xs = 2 then
        let varName = List.item 0 xs |> function
          | Symbol t -> t
