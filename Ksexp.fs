...
module Ksexp =
    ...
    let nextBracket code leftOffset =
        ...
        let rec loop index leftCount rightCount =
            if index < String.length code && leftCount <> rightCount then
                if code.[index] = '('
    ...
    let rec parseList (str: string) =
        ...
        let content = str.[1..nextBracketIdx - 1]
    ...
    and parseNumber str =
        let dotNextIsNumber (str: string) strLen i = (str.[i] = '.' && i + 1 < strLen && isDigit (str.[i + 1]))
        let i = ref 0
        let strLen = String.length str
        ...
        let rec loop i =
            if i < strLen && (isDigit (str.[i]) || dotNextIsNumber str strLen i)
            then loop (i + 1)
            else i
    ...
    and parseSymbol str =
        ...
        let rec loop i =
            if i < strLen && (isAlpha str.[i] || Set.exists ((=) str.[i]) symbolChars)
            then loop (i + 1)
            else i
            ...
            if i < codeLen then
                ...
                | ';' ->
                    let j =
                        ...
                        t.readLen
                    loop (i + j)
                | c when isDigit c || (c = '-' && i + 1 < codeLen && isDigit code.[i + 1]) ->
